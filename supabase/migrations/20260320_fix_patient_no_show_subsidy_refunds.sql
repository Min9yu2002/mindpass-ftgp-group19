...
BEGIN
  ...
  -- refundable. Patient no-show returns 50% of funded subsidy and keeps the other
  -- 50% consumed as therapist compensation. Other unwind states refund funded
  -- subsidy in full. Completed keeps subsidy consumed.
  ...
  ELSIF NEW.status = 'patient_no_show' THEN
    target_refund_total := round(effective_funded_amount * 0.5, 18);
  ELSIF NEW.status IN (
    'rejected',
    'payment_timeout',
    'therapist_no_show',
...
WITH eligible_refunds AS (
  SELECT
    ...
    greatest(
      0::numeric,
      round(coalesce(s.subsidy_funded_eth, 0) * 0.5, 18) - coalesce(s.subsidy_refunded_eth, 0)
    ) AS refund_increment
