...
/// @author M1n9yu
/// @notice Session escrow and settlement contract for MindPass.
/// @dev Supports dual-source funding where the patient funds the wallet portion
/// and the platform vault funds the subsidy portion. All payouts and refunds use
/// pull payments through claimable balances.
contract MindPassEscrow is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public constant EARLY_CANCEL_WINDOW = 5 minutes;
    uint256 public constant PAYMENT_WINDOW = 3 minutes;
    uint256 public constant NO_SHOW_WINDOW = 20 minutes;
    ...
    error TooEarlyToCancelUnstarted(uint256 sessionId, uint256 earliestCancelAt);
    error CancellationWindowClosed(uint256 sessionId, uint256 noShowDeadlineAt);
    error SessionAlreadyHasAttendance(uint256 sessionId);
    ...
    event SessionCancelledUnstarted(
        uint256 indexed sessionId,
    ...
    /// @notice Creates a booking request with an explicit patient/vault funding split.
    /// @param therapist Therapist wallet for the requested session.
    /// @param walletRequiredWei Portion to be funded by the patient wallet.
    /// @param subsidyRequiredWei Portion to be funded by the platform vault.
    /// @param sessionMode Arbitrary session mode identifier such as bytes32("text").
    ...
    /// @notice Accepts a requested booking and starts the payment window.
    ...
    /// @param sessionId Session to reject.
    ...
    /// @notice Funds the patient wallet portion of a booking.
    /// @param sessionId Session to fund.
    ...
    /// @notice Funds the subsidy portion of a booking from the platform vault.
    /// @param sessionId Session to fund.
    ...
    /// @notice Resolves an accepted booking that missed its payment deadline.
    /// @param sessionId Session to time out.
    ...
    /// @notice Cancels a fully funded but unstarted session after the early exit window.
    /// @param sessionId Session to cancel.
    function cancelUnstartedSession(uint256 sessionId) external whenNotPaused {
        Session storage session = _getSession(sessionId);
        ...
        uint256 earliestCancelAt = session.fundedAt + EARLY_CANCEL_WINDOW;
        if (session.fundedAt == 0 || block.timestamp < earliestCancelAt) {
            revert TooEarlyToCancelUnstarted(sessionId, earliestCancelAt);
        }
        if (block.timestamp >= session.noShowDeadlineAt) {
            revert CancellationWindowClosed(sessionId, session.noShowDeadlineAt);
        }
        if (
            session.patientJoinedAt != 0 || session.therapistJoinedAt != 0 || session.sessionStartedAt != 0
        ) {
            revert SessionAlreadyHasAttendance(sessionId);
        }

        _applyCancelledUnstartedSettlement(sessionId, session, msg.sender);
    }

    /// @notice Marks the patient as present for a funded live session.
    ...
    /// @notice Marks the therapist as present for a funded live session.
    /// @param sessionId Session to join.
    ...
    /// @notice Resolves a funded session after the no-show deadline passes.
    ...
    /// @notice Completes an in-session booking and releases claimable balances.
    /// @param sessionId Session to complete.
    function completeSession(uint256 sessionId) external whenNotPaused {
        Session storage session = _getSession(sessionId);
        if (msg.sender != session.patient && msg.sender != session.therapist) {
            revert UnauthorizedCaller(sessionId, msg.sender);
        }
    ...
    /// @notice Withdraws the caller's accumulated claimable balance.
    ...
    /// @notice Updates the platform vault used to fund subsidy portions and receive subsidy refunds.
    ...
    /// @notice Updates the treasury address that accrues protocol fees.
    ...
    /// @notice Pauses mutating session lifecycle actions.
    function pause() external onlyOwner {
        _pause();
    }

    /// @notice Unpauses mutating session lifecycle actions.
    ...
    function _applyCancelledUnstartedSettlement(
        uint256 sessionId,
        Session storage session,
        address cancelledBy
    ) internal {
        _assertDistribution(session.feeWei, SESSION_FEE, 0, 0);
        ...
        session.status = SessionStatus.CancelledUnstarted;
        session.cancelledAt = block.timestamp;
        session.penaltyFeeWei = 0;
        session.refundAmountWei = SESSION_FEE;
        session.protocolFeeWei = 0;
        session.therapistPayoutWei = 0;
        ...
        emit SessionCancelledUnstarted(
            sessionId,
            cancelledBy,
            patientRefundWei,
            vaultRefundWei,
